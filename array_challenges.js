...
const movies = [
  ...
  */
  ...
  // ✍️ Solve it here ✍️
  const movie = movies.map(users => ({
    title: users.title,
    rating: users.rating/10
  }));
  
  movie.forEach(user => {
    console.log(`Title: [${user.title} - Rating: ${user.rating}]`);
  });
  ...
  */
  ...
  const filteredMovies = movies.filter(movie => movie.rating >= 8.0);
